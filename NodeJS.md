...
## 2.4 Retrieving data (simple select statement) (required level 3)
  ...
  ***level 4:***
  ...
  - Knowledge of subqueries  — это запрос, вложенный в инструкцию SELECT, INSERT, UPDATE или DELETE или в другой подзапрос.;
...
## 2.6 CRUD concept (required level 4)
 ...
 > Операторы Transact SQL можно разделить на операторы DDL, DML и DCL, а с точки зрения компьютерного программирования операторы DML можно назвать операциями CRUD. В этой статье мы можем более подробно рассмотреть операции CRUD и то, как мы можем использовать эти операторы в SQL Server, с примерами.
 > CRUD — это аббревиатура от операторов CREATE, READ(SELECT), UPDATE и DELETE в SQL Server. CRUD в терминах базы данных также может упоминаться как операторы языка манипулирования данными (DML). Язык манипулирования данными используется для управления или манипулирования данными, присутствующими в таблицах базы данных.
